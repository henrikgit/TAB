...
Before a deeper explanation is provided, I will answer the question why there is such a column which serves to prohibit summation of the budgeted amount in the first place.
...
I do not know whether I solved the problem in the best way possible, but it certainly works for dealing with 'taking the money out of the envelope' which is the inherent characteristic to the envelope method.

Because, as described in \autoref{subsubsec:budgeting-item-column-monthly-expense}, there are also monthly expenses, which are not to be budgeted via the typical means of the envelope method.
...
So, putting \programmcode{Yes} is a must if you have a monthly expense, otherwise all the amounts per month get summed up to their total in the last month, which would lead to strong deviations in each of the relevant months.
% sense if the item's time period is \emph{longer} than 1 month, which typically happens if it is a monthly expense.

Putting \programmcode{No} makes sense if the expense is a one-time purchase.
...
There is also \autoref{subsec:examples-budgeting-expenses} starting on \autopageref{subsec:examples-budgeting-expenses}, which serves to clarify some budgeting ideas and concepts with examples.
