...
Additional remarks and relatively deeper thoughts can be found in \autoref{subsec:advanced-tips-for-budgeting}, \autopageref{subsec:advanced-tips-for-budgeting}.
...
The values which are then used can be In rough terms, they are split into 2 categories:\todo{weiter}
\begin{enumerate}
...
The date you enter must have the format \codestuff{YYYY-MM}, which are a shorter version of the dates in the tracking sheets (for more see \autoref{subsec:tracking-column-date}, \autopageref{subsec:tracking-column-date}).
...
This will be elaborated on in \autoref{subsubsec:multiple-non-monthly-expenses}, \autopageref{subsubsec:multiple-non-monthly-expenses}.
...
It obviously must be set to the opposite value of the previous column, as \autoref{tab:possible-settings-monthly-expense-sum-prohibition} on \autopageref{tab:possible-settings-monthly-expense-sum-prohibition} 
shows.
\begin{itemize}
	\item Put \codestuff{Yes} if you want to budget for a single expense.
	\item Put \codestuff{No} if you want the budget for multiple expenses.
\end{itemize}
...
At this point in time, the expense item is \sterm{fully budgeted} or \sterm{fully saved up} and can be purchased/billed.
...
To conclude:
\begin{itemize}
	\item Putting \codestuff{No} is a must if you have a monthly expense.
	...
	\item Putting \codestuff{Yes} makes sense if the expense is a one-time purchase.
...
Now that the main explanations in regards to monthly expenses and sum prohibition were given, will list the possible combinations and use cases for both settings.
\begin{table}[hbtp]
	\centering
...
\end{itemize}
The entry must be either \codestuff{Yes} or \codestuff{No}, or the formula will a value of 0.
